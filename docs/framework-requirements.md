...
|---|---|---|---|---|
| FW-REQ-010 | `python-can` | L0 hardware abstraction (TOOL-REQ-001–007) | BSD-2-Clause [EST] | Reuse as-is; this *is* the L0 layer |
| FW-REQ-011 | `cantools` | DBC/ARXML/A2L decode (TOOL-REQ-014, 015, 017) | MIT [EST] | Reuse; contribute upstream fixes for ARXML-depth gaps found during integration rather than forking |
| FW-REQ-012 | `python-can-isotp` | ISO-TP transport (TOOL-REQ-022) | LGPL-3.0 [EST — **verify before shipping**; if copyleft, isolate as an optional/pluggable transport rather than a hard-linked core dependency to protect the Apache-2.0 core, per FW-REQ-014] | Verify license at implementation time; do not assume MIT/BSD |
| FW-REQ-013 | `udsoncan` | UDS client (TOOL-REQ-022, 023, 024) | MIT [EST] | Reuse; wrap, don't rewrite |
| FW-REQ-014 | `doipclient` | DoIP transport (TOOL-REQ-023) | MIT [EST] | Reuse; wrap into the `udsoncan` connection |
| FW-REQ-015 | `asammdf` | MDF4 read/write (TOOL-REQ-021) | LGPL-3.0 [EST — **verify**; same isolation caveat as FW-REQ-012] | Verify license at implementation time |
| FW-REQ-016 | `pytest` | Test runner (TOOL-REQ-028–030) | MIT [EST] | Native — this *is* the L3 differentiation vs. CAPL |
| FW-REQ-017 | License verification gate (process requirement) | Must | Before any new dependency is added, its license must be confirmed against the policy in §6 (permissive-only for the OSS core) — `python-can-isotp` and `asammdf` are flagged above as needing this check explicitly, since LGPL is a plausible license for both and would require isolation, not just acceptance | `IV-LIC-04` |
...
| FW-REQ-032 | Maintain meaningful automated-test coverage on L0–L2 | Should | [NEW-EST — no source specifies a numeric target; maintainers should set one, e.g. 80%+ on L0–L2, rather than leave it open] |
